...
business = sqlContext.read.load('yelp/yelp_academic_dataset_business.json', format='json')
review = sqlContext.read.load('yelp/yelp_academic_dataset_review.json', format='json')

business_Pittsburgh = business.filter((business['city'] == 'Pittsburgh') & (business['state'] == 'PA'))
business_id_Pittsburgh = business_Pittsburgh.rdd.map(lambda row: row['business_id']).collect()

review_Pittsburgh = review.filter(review['business_id'].isin(business_id_Pittsburgh))
data = review_Pittsburgh.select('text')
...
tokens = data.rdd.map(lambda row: row['text'])              \
